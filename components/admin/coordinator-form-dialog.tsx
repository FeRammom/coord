...
export function CoordinatorFormDialog({ mode, coordinator }: CoordinatorFormDialogProps) {
  ...
  const action = mode === 'create' ? createCoordinatorAction : updateCoordinatorAction
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          <DialogTitle>
            {mode === 'create' ? 'Новый координатор' : 'Редактировать координатора'}
          </DialogTitle>
        </DialogHeader>
        <form action={formAction} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Input id="email" name="email" type="email" defaultValue={coordinator?.email || ''} />
          </div>
          <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Input id="organization" name="organization" defaultValue={coordinator?.organization || ''} />
          </div>
